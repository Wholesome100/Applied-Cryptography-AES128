...
//Common functions done by Orfield
...
{
        for (unsigned int i=0; i<16; i++)//XOR each byte in the byte_array with the key
        {
        byte_array[i] ^= key[i];
        }
}
...
{
    const byte sbox[256]={0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F, 0xC5, 0x30, 0x01, 0x67, 0x2B, 0xFE, 0xD7, 0xAB, 0x76,
    ...
    int index = byte_to_substitute;//Cast byte to an int value index

    return sbox[index];//Return the value at index
}
...
void substitute_bytes(byte *byte_array)
{
    ...
    {
        byte_array[i] = substitute_byte(byte_array[i]);//Sub the bytes of a state
